...
FCLoginResult = namedtuple(
    "FCLoginResult", ["success", "user_id", "username", "email", "md5password", "error"]
)
...
def fc_login(username: str, password: str) -> FCLoginResult:
    ...
    try:
        ...
        try:
            ...
            username_node = dom.firstChild.getElementsByTagName("userName")[
                0
            ].firstChild.toxml()
            user_id_str = dom.firstChild.getElementsByTagName("userId")[
                0
            ].firstChild.toxml()
            md5password_node = dom.firstChild.getElementsByTagName("password")[
                0
            ].firstChild.toxml()
            email_node = dom.firstChild.getElementsByTagName("email")[
                0
            ].firstChild.toxml()
...
def _struct_to_save_design_xml(
    design_struct: FCDesignStruct, user_id: Optional[int], name: str, description: str
) -> minidom.Document:
...
def _piece_struct_to_xml(piece, dom) -> minidom.Element:
    ...
    # Goal block
    is_goal = piece.type_id in (
        fcsim_piece_types.FCSIM_GP_RECT.value,
        fcsim_piece_types.FCSIM_GP_CIRC.value,
    )
