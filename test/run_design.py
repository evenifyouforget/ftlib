...
RunDesignResult = namedtuple(
    "RunDesignResult", ["proc", "real_solve_ticks", "real_end_ticks"]
)
...
def run_design(
    ...
    all_pieces = (
        design_struct.goal_pieces
        + design_struct.design_pieces
        + design_struct.level_pieces
    )
    ...
    for i, piece_struct in enumerate(all_pieces):
        ...
        serialized_input.append(
            piece_struct.piece_id if piece_struct.piece_id is not None else i + 1000
        )
    ...
    try:
        ...
        raise AssertionError(
            f"Process {debug_command_text} timed out after {timeout:.1f}s (max_ticks={max_ticks})"
        )
    if proc.returncode != 0:
        raise AssertionError(
            f"Process {debug_command_text} exited with return code {proc.returncode}"
        )
    stdout = proc.stdout
