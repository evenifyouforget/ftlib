...
def generate_test_single_design_data():
    ...
    for row in fc_data:
        ...
        if solve_ticks is None and design_max_ticks is None:
            raise ValueError(
                f"No time to solve or time to not solve specified for: {design_uid}"
            )
        # generate basic data
        design_uid = f"D{design_id}" if design_id else f"L{level_id}"
        # placeholder
...
def pytest_generate_tests(metafunc):
    if "design_uid" in metafunc.fixturenames:
        metafunc.parametrize(
            "design_uid,design_data", generate_test_single_design_data()
        )
...
def test_single_design(design_uid, design_data, global_max_ticks, use_classic_timeout):
    ...
    correct_result_message = (
        f"solves at {expect_solve_ticks}"
        if expect_solve_ticks is not None
        else f"still no solve within {design_max_ticks}"
    )
    post_message = (
        f' (design "{design_struct.name}")'
        if "D" in design_uid
        else f' (level "{design_struct.name}")'
    )
    if user_comment:
        post_message += f" (user comment: {user_comment})"
    real_result_message = (
        f"Design solved at {real_solve_ticks}"
        if real_solve_ticks != -1
        else f"Design did not solve within {real_end_ticks}"
    )
    error_message = (
        f"{real_result_message} (expected: {correct_result_message}){post_message}"
    )
    if expect_solve_ticks is None or expect_solve_ticks > real_end_ticks:
        ...
        if real_solve_ticks != expect_solve_ticks:
            raise AssertionError(error_message)
    if (
        not use_classic_timeout
        and real_solve_ticks == -1
        and real_end_ticks
        < (expect_solve_ticks if expect_solve_ticks is not None else design_max_ticks)
    ):
        # still uncertain
        pytest.skip(
            f"Ran to {real_end_ticks} and still no solve (expected: {correct_result_message}){post_message}"
        )


# test single designs using fcsim backend
def test_single_design_fcsim(
    design_uid, design_data, global_max_ticks, use_classic_timeout
):
    # unpack tuple
    design_struct = design_data.design_struct
    expect_solve_ticks = design_data.expect_solve_ticks
    ...
    if max_ticks == math.inf:
        max_ticks = -1  # program treats -1 as infinity
    # run the design using fcsim backend
    run_result = run_design(design_struct, max_ticks, backend="fcsim")
    # unpack result
    real_solve_ticks = run_result.real_solve_ticks
    real_end_ticks = run_result.real_end_ticks
    # check result
...
def test_valgrind(design_uid, design_data, global_max_ticks):
    ...
    run_result = run_design(
        design_struct,
        max_ticks,
        command_prepend=["valgrind"],
        timeout_multiplier=VALGRIND_TIMEOUT_MULTIPLIER,
    )
