...
std::istream &operator>>(std::istream &is, fcsim_block_def &block) {
  int64_t block_type;
  is >> block_type >> block.id >> block.x >> block.y >> block.w >> block.h >>
      block.angle >> block.joints[0] >> block.joints[1];
...
// extremely low level (not for human use) program to run a single design, and
// see if it solves
int main() {
