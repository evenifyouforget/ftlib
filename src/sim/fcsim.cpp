...
static block *find_block_by_id(const std::shared_ptr<ft_sim_state> handle,
                               int id) {
...
static joint_collection *
get_closest_jc(const std::shared_ptr<ft_sim_state> handle, double x, double y,
...
void get_rod_endpoints(fcsim_block_def bdef, double *x0, double *y0, double *x1,
                       double *y1) {
...
int share_block(const std::shared_ptr<ft_sim_state> handle,
                const joint_collection *jc0, const joint_collection *jc1) {
  for (int i = 0; i < handle->block_cnt; i++) {
    bool f0 = false, f1 = false;
    for (joint_collection_list *j = handle->blocks[i].jcs_head; j;
         j = j->next) {
      if (j->jc == jc0)
        f0 = true;
      if (j->jc == jc1)
...
static bool is_singular_wheel_center(const joint_collection *jc) {
  ...
  return jc->x == jc->top_block->bdef.x && jc->y == jc->top_block->bdef.y;
}
...
static void replace_joint(block *b, joint_collection_list *jcl,
                          joint_collection *jc) {
  delete jcl->jc;
  jcl->jc = jc;
...
static void create_rod_joints(block *b,
                              const std::shared_ptr<ft_sim_state> handle) {
  fcsim_block_def bdef = b->bdef;
  double x0, y0, x1, y1;
...
static void create_wheel_joints(block *b,
                                const std::shared_ptr<ft_sim_state> handle) {
  fcsim_block_def bdef = b->bdef;
  double x = bdef.x;
  double y = bdef.y;
...
static void create_joints(block *b,
                          const std::shared_ptr<ft_sim_state> handle) {
  switch (b->bdef.type) {
  case fcsim_piece_type::GP_RECT:
...
void add_block(const std::shared_ptr<ft_sim_state> handle,
               fcsim_block_def bdef) {
...
void fcsim_step(std::shared_ptr<ft_sim_state> handle,
                const ft_sim_settings &settings) {
...
bool fcsim_in_area(const fcsim_block_def &bdef, const fcsim_rect &area) {
  ...
  return ft_sub(bdef.x, bex2) >= area_xa && ft_add(bdef.x, bex2) <= area_xb &&
         ft_sub(bdef.y, bey2) >= area_ya & ft_add(bdef.y, bey2) <= area_yb;
}

bool fcsim_is_solved(const std::shared_ptr<ft_sim_state> sim,
                     const ft_design_spec &spec) {
  bool goal_exist = false;
  for (int i = 0; i < sim->block_cnt; ++i) {
    fcsim_block_def &bdef = sim->blocks[i].bdef;
    if (!is_goal_object(bdef.type))
      continue;
    goal_exist = true;
    if (!fcsim_in_area(bdef, spec.goal))
      return false;
  }
  return goal_exist;
}
