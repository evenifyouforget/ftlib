...
{
	b2AssertMemory(b2IsPowerOfTwo(b2_tableCapacity) == true);
	b2AssertMemory(b2_tableCapacity >= b2_maxPairs);
...
b2Pair* b2PairManager::Find(int32 proxyId1, int32 proxyId2, uint32 hash)
{
	...
	b2AssertMemory(index < b2_maxPairs);
...
b2Pair* b2PairManager::AddPair(int32 proxyId1, int32 proxyId2)
{
	...
	b2AssertMemory(m_pairCount < b2_maxPairs && m_freePair != b2_nullPair);
...
void b2PairManager::AddBufferedPair(int32 id1, int32 id2)
{
	...
	b2AssertMemory(m_pairBufferCount < b2_maxPairs);
...
void b2PairManager::Commit()
{
	...
	for (int32 i = 0; i < m_pairBufferCount; ++i)
	{
		...
		b2AssertMemory(pair->proxyId1 < b2_maxProxies && pair->proxyId2 < b2_maxProxies);
...
void b2PairManager::ValidateBuffer()
{
	...
	for (int32 i = 0; i < m_pairBufferCount; ++i)
	{
		...
		b2AssertMemory(pair->proxyId1 < b2_maxProxies);
		b2AssertMemory(pair->proxyId2 < b2_maxProxies);
