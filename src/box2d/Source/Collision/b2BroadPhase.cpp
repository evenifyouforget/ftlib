...
bool b2BroadPhase::TestOverlap(b2Proxy* p1, b2Proxy* p2)
{
	for (int32 axis = 0; axis < 2; ++axis)
	{
		...
		b2AssertMemory(p1->lowerBounds[axis] < 2 * m_proxyCount);
		b2AssertMemory(p1->upperBounds[axis] < 2 * m_proxyCount);
		b2AssertMemory(p2->lowerBounds[axis] < 2 * m_proxyCount);
		b2AssertMemory(p2->upperBounds[axis] < 2 * m_proxyCount);
...
bool b2BroadPhase::TestOverlap(const b2BoundValues& b, b2Proxy* p)
{
	for (int32 axis = 0; axis < 2; ++axis)
	{
		...
		b2AssertMemory(p->lowerBounds[axis] < 2 * m_proxyCount);
		b2AssertMemory(p->upperBounds[axis] < 2 * m_proxyCount);
...
void b2BroadPhase::ComputeBounds(uint16* lowerValues, uint16* upperValues, const b2AABB& aabb)
{
	...
	lowerValues[0] = (uint16)(m_quantizationFactor.x * (minVertex.x - m_worldAABB.minVertex.x))._v & (USHRT_MAX - 1);
	upperValues[0] = (uint16)(m_quantizationFactor.x * (maxVertex.x - m_worldAABB.minVertex.x))._v | 1;

	lowerValues[1] = (uint16)(m_quantizationFactor.y * (minVertex.y - m_worldAABB.minVertex.y))._v & (USHRT_MAX - 1);
	upperValues[1] = (uint16)(m_quantizationFactor.y * (maxVertex.y - m_worldAABB.minVertex.y))._v | 1;
}
...
void b2BroadPhase::IncrementOverlapCount(int32 proxyId)
{
	...
	else
	{
		...
		b2AssertMemory(m_queryResultCount < b2_maxProxies);
...
{
	b2AssertMemory(m_proxyCount < b2_maxProxies);
	...
	{
		b2AssertMemory(m_queryResults[i] < b2_maxProxies);
...
{
	b2AssertMemory(0 < m_proxyCount && m_proxyCount <= b2_maxProxies);
	b2Proxy* proxy = m_proxyPool + proxyId;
