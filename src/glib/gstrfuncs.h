...
GLIB_AVAILABLE_IN_ALL
gchar *g_strrstr_len(const gchar *haystack, gssize haystack_len,
                     const gchar *needle);
...
#define g_str_has_prefix(STR, PREFIX)                                          \
  (__builtin_constant_p(PREFIX) ? G_GNUC_EXTENSION({                           \
    ...
    if G_UNLIKELY (__str == NULL || __prefix == NULL)                          \
      ...
      if (__str_len >= __prefix_len)                                           \
        __result = memcmp(_G_STR_NONNULL(__str), _G_STR_NONNULL(__prefix),     \
                          __prefix_len) == 0;                                  \
    }                                                                          \
    __result;                                                                  \
  })                                                                           \
...
#define g_str_has_suffix(STR, SUFFIX)                                          \
  (__builtin_constant_p(SUFFIX) ? G_GNUC_EXTENSION({                           \
    const char *const __str = (STR);                                           \
    ...
    if G_UNLIKELY (__str == NULL || __suffix == NULL)                          \
      ...
      if (__str_len >= __suffix_len)                                           \
        __result = memcmp(__str + __str_len - __suffix_len,                    \
                          _G_STR_NONNULL(__suffix), __suffix_len) == 0;        \
    }                                                                          \
    __result;                                                                  \
  })                                                                           \
...
GLIB_AVAILABLE_IN_2_54
gboolean g_ascii_string_to_signed(const gchar *str, guint base, gint64 min,
                                  gint64 max, gint64 *out_num, GError **error);

GLIB_AVAILABLE_IN_2_54
gboolean g_ascii_string_to_unsigned(const gchar *str, guint base, guint64 min,
                                    guint64 max, guint64 *out_num,
                                    GError **error);

/**
 * g_set_str: (skip)
 * @str_pointer: (inout) (not optional) (nullable): a pointer to either a string
 * or %NULL
